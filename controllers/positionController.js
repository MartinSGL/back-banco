...
module.exports = {
    async index(req,res){
        try{
            let data = await position.findAll({}) //buscar todos los registros con deletedAt = null
    ...
    async create(req,res){
        try{
            ...
            let data = await position.create(req.body);
            return res.status(OK).json(resOk(data));
        }catch(err){
            //si se comete un error mandar un status ERROR = 400
            return res.status(ERROR).send(resError(error));
    ...
    async update(req,res){
        try{
            ...
            let idFound = await position.findOne({where:{id}}) //comprobar si existe el id en la base de datos
            ...
            //los parametros enviados se deben llamar igual en base de datos y desde el formulario enviado (name)
            let [,data] = await position.update(req.body,{

...
module.exports = {
    async login(req,res){
        try{
            ...
            let executiveF = await executive.findOne({where: {userid},include:[{model:position}]})
            ...
            if(permision===true) {
                console.log(executiveF)
                //agregar información al token
                const name = `${executiveF.name} ${executiveF.lastname}`
                const payload = {session:{ id:executiveF.id,name,rol:executiveF.Position.name}}
